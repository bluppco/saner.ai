...
const BenchmarkingJSX = (props) => {
  const formatNumber = (number) => {
    if (number < 100) return number.toFixed(2) + " MB";
    else if (number / 1000 < 1000) return (number / 1000).toFixed(2) + " GB";
      ...
      <Table>
        ...
        <TableBody>
          {data.data.map((value, index) => {
            return (
              <TableRow
                className={value.type === "Total" ? "font-semibold" : ""}
                key={"type-split-" + index}
              >
                ...
                <TableCell className="text-center">
                  {(value.current_size / 1000).toFixed(2)}
                </TableCell>
                <TableCell className="text-center">
                  {value.blupp_size}
                </TableCell>
                <TableCell className="text-center">
                  {(value.blupp_size / 1000).toFixed(2)}
                </TableCell>
                <TableCell className="text-center">
                  {((value.current_size - value.blupp_size) / 1000).toFixed(2)}
                </TableCell>
                <TableCell className="text-center">
                  {(
                    ((value.current_size - value.blupp_size) /
                      value.current_size) *
                    100
                  ).toFixed(2)}
                </TableCell>
        ...
        <TableBody>
          {data.bandwidth.map((value, index) => {
            return (
              <TableRow
                className={value.type === "Total" ? "font-semibold" : ""}
                key={"bandwidth-saved-" + index}
              >
                <TableCell className="font-semibold">
                  {value.hits_text}
                </TableCell>
                <TableCell className="text-right">
                  {formatNumber(value.current_size * value.hits)}
                </TableCell>
                <TableCell className="text-right">
                  {formatNumber(value.blupp_size * value.hits)}
                </TableCell>
                <TableCell className="text-right">
                  {formatNumber(
                    value.current_size * value.hits -
                      value.blupp_size * value.hits,
                  )}
                </TableCell>
                <TableCell className="text-right">
                  {formatPrice(value.current_size * value.hits, 0.1, 1000000)}
                </TableCell>
                <TableCell className="text-right">
                  {formatPrice(value.current_size * value.hits, 0.4, 1000000)}
                </TableCell>
                <TableCell className="text-right">
                  {formatPrice(value.current_size * value.hits, 0.2, 500000)}
                </TableCell>
                <TableCell className="text-right font-semibold">
                  {formatPrice(value.blupp_size * value.hits, 0.05, 1000000)}
                </TableCell>
              </TableRow>
            );
          })}
